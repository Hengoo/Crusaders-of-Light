...
Shader "Custom/SnowShaderColor" {
	Properties{
		...
		_NoiseMulti("NoiseFactor", Range(0.0,2.0)) = 1.0
		_NoiseAdd("NoiseFactor2", Range(-1.0,1.0)) = 0.0
	}
		SubShader{
	...
	#define WorldToTangentNormalVector(data,normal) normal
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				//if (topBlend != 0 && topBlend != 1)
				{
					...
					/*
					half height = tex2D(_HeightMap, IN.uv_MainTex) * (1 - topBlend);
					half heightTop = (tex2D(_HeightMapUp, uvXTop) * triblend.x + tex2D(_HeightMapUp, uvYTop) * triblend.y + tex2D(_HeightMapUp, uvZTop) * triblend.z) * topBlend;

					half noiseFactor = tex2D(_NoiseTex, uvXTop*_NoiseTex_ST) *triblend.x + tex2D(_NoiseTex, uvYTop*_NoiseTex_ST) *triblend.y + tex2D(_NoiseTex, uvZTop*_NoiseTex_ST) *triblend.z;
					noiseFactor += _NoiseAdd;
					topBlend = clamp( topBlend + (heightTop - height) *_HeightFactor, 0 ,1) * clamp((1-noiseFactor*noiseFactor*_NoiseMulti),0,1);
					*/


					//version without height:
					half noiseFactor = tex2D(_NoiseTex, uvXTop*_NoiseTex_ST) * triblend.x + tex2D(_NoiseTex, uvYTop*_NoiseTex_ST) * triblend.y + tex2D(_NoiseTex, uvZTop*_NoiseTex_ST) * triblend.z;
					noiseFactor += _NoiseAdd;
					topBlend = topBlend * clamp((1-noiseFactor * noiseFactor * _NoiseMulti),0,1);
